...
void add_memory_block(int adresse_debut, int taille) {
    if (num_blocks >= TAILLE_MEMOIRE_MAX) {
        printf("\tLe tableau de blocs de la mémoire est plein\n");
        return;
    }
    memory_blocks[num_blocks].adresse_debut = adresse_debut;
    memory_blocks[num_blocks].end_address = adresse_debut + taille - 1;
    memory_blocks[num_blocks].taille = taille;
    memory_blocks[num_blocks].is_allocated = 0;
    num_blocks++;
}
int best_fit_allocate(int taille) {
    int best_fit_index = -1;
    int taille_min = TAILLE_MEMOIRE_MAX + 1;
    for (int i = 0; i < num_blocks; i++) {
        if (!memory_blocks[i].is_allocated && memory_blocks[i].taille >= taille && memory_blocks[i].taille < taille_min) {
            best_fit_index = i;
            taille_min = memory_blocks[i].taille;
        }
    }
    if (best_fit_index != -1) {
        memory_blocks[best_fit_index].is_allocated = 1;
        return memory_blocks[best_fit_index].adresse_debut;
    }
    return -1;
}
...
int main() {
    ...
    int address4 = best_fit_allocate(10);
    if (address4 != -1) {
        printf("The allocated block with taille 10 at address %d\n", address4);
    }
    return 0;
